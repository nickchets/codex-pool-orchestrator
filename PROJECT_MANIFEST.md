...
- The current Gemini sequencing is explicit after the Antigravity audit: env-backed managed OAuth/runtime truth comes first, then provider-truth persistence and warm-seat admission, then Gemini routing/cooldown logic, and only after that broader UI parity on `/status` or `/`.
...
- The Antigravity transfer is pattern-level only: sticky current-seat preference, provider-truth quota state, warm-seat admission, and core backoff may be imported, but Antigravity file indices, sqlite bookkeeping, log-scrape guards, and hidden fixed-account behavior must not become a second control plane inside this repo.
...
- The follow-on route refreshed on 2026-03-25 is now explicit as well: after `T46/T47`, freeze the quota-first Gemini contract and shim semantics (`T49`), align Gemini CLI/OpenCode connectivity and exported limits truth (`T50`), then land operator/dashboard parity (`T44`), and only after that run any controlled live seat delete/reimport proof (`T51`).
...
- The next live boundary closed on 2026-03-27 as well: `T47` now blocks stale provider truth / stale quota snapshots, `missing_project_id`, not-warmed restricted seats, quota pressure, and hard operational failure before Gemini selection, while startup stale-truth refresh re-hydrates ready Antigravity seats so the new gate does not collapse the pool after restart.
- The client/dashboard parity wave was live-closed on 2026-03-27 as well: Gemini CLI now stays on the pool root URL in external API-key mode, OpenCode exports `/v1` plus `antigravity-accounts.json` with an enabled `activeIndex`, and the landing/status surfaces consume the same `gemini_pool` / `provider_quota_summary` / `compatibility_lane` truth on the restarted service.
...
- The post-release closure chain on 2026-03-27 is now explicit and mostly complete: `T55` re-proved a fresh browser-auth seat on the published binary, `T58` repaired the false-negative operator `gemini-3.1-pro` seat-smoke path, and `T56` made `agcode` / OpenCode the canonical no-prompt local entrypoint with exported `model=antigravity-manager/gemini-3.1-pro` plus a re-proved ready-seat `activeIndex` on the live closure snapshot.
- The remaining repo-local Gemini work is therefore narrower than the earlier roadmap packet: only `T57` remains from that chain, and the active in-progress sub-slice is now specifically cooldown/stale-quota/model-specific rate-limit taxonomy, precise provider-derived reset windows, and richer operator/export diagnostics rather than any client bootstrap or browser-auth proof gap.
