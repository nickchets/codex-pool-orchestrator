...
- The repo-local Gemini closure chain is now complete through `0.8.7`: `T57` closed the last bounded repository slice around cooldown/stale-quota/model-specific rate-limit taxonomy, precise provider-derived reset windows, and richer operator/export diagnostics, while the follow-on publish wave also cleaned the canonical browser-auth/OpenCode operator contract, Codex refresh-invalid health truth, and the published `/operator/gemini/oauth-start` source route. Any further follow-up is narrower operational/provider truth or a fresh repo-local card, not a still-open continuation of `T57`.
...
- Root card `ROOT-E30-S1-T5` now anchors the remaining post-release Gemini limits/accounts/observability complaint on the control-plane side: if that follow-up reopens into product work after `0.8.7`, claim a fresh repo-local Gemini operator card from this capsule's board/evidence and use this manifest note as the handoff anchor instead of routing the complaint back through `ROOT-AUTO-INTAKE-T1`, reopening finished root-core cards, or silently reviving already-closed repo-local Gemini closure cards such as `REPO-CPO-ARCH-P2-T57`.
- That fresh repo-local continuation is now explicit: `REPO-CPO-UX-P3-T59` owns the screenshot-first `/` + `/status` audit/spec wave from `DIR-20260328-040`, so the landing-vs-deep-ops information-architecture work continues here instead of reopening `ROOT-E30-S1-T5` or reviving synthetic root auto-intake residue.
- The post-`0.8.7` cleanup wave closed on `2026-03-29` as `0.9.0`: the screenshot-first audit is now documented in repo-local docs, `/` is the canonical onboarding/dashboard surface, `/status` is explicitly diagnostics-only, the landing exposes Codex quota freshness/reset timing without seat-identity overflow, OpenCode exports the broader Gemini catalog while keeping `codex-pool/gemini-3.1-pro-high` as default, and Codex sticky selection no longer ejects refreshable expired seats prematurely.
