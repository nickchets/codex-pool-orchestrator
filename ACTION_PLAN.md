...
- Done: `/status?format=json` now exports `provider_truth.rate_limit_reset_times` for live Gemini model cooldowns, so the seat-level status JSON exposes the same per-model reset truth that already drove `agcode` / OpenCode export and merged quota rows.
...
- Done: warmed Antigravity Gemini seats that still converge to `provider_truth.state=missing_project_id` now stay export-visible as `routing.state=degraded_enabled` with `fallback project in use; provider truth missing project_id`, and their `provider_truth.quota.models[]` rows stay `routable=true` in both `/status` and OpenCode cached quota instead of drifting into a false seat-blocked picture.
