...
1. Operator directive on `2026-03-27` is explicit: do not cut a narrow Gemini publish. Close the current Gemini/browser-auth/OpenCode tail through repo-local bureaucracy and publish once with version/changelog only after the runtime, client, dashboard, and seat-reset slices are finished.
...
3. The combined Gemini/browser-auth/OpenCode + observability wave is now publish-ready as `0.8.0`: full verify is green on the release candidate binary, GitLab Claude recovery (`T53`) is no longer blocking, and the remaining truthful residue is narrowed to post-release operational follow-up (`T55`) rather than to any unresolved release gate.
4. The post-release audit wave on `2026-03-27` converged on the execution order `T55 -> T58 -> T56 -> T57`: first re-prove one fresh browser-auth add on the published binary, then repair the false-negative `gemini-3.1-pro` operator smoke path it exposed, then make OpenCode via `codex-pool/gemini-3.1-pro-high` the canonical no-prompt local entrypoint, and only after that pay down the broader Gemini admission / dead-source brittleness.
5. The same follow-through wave on `2026-03-27` is now closed in repo-local truth as well: `T55`, `T58`, `T56`, and `T57` all passed with fresh live proof on the running pool and are published through `0.8.7`.
6. The one still-live `missing_project_id` Gemini seat is no longer treated as a repo-local blocker. After the `0.8.7` closure audit, it is classified as operational/provider truth: the pool exports/routs it truthfully through the degraded fallback-project lane, and further “fix” work would require provider-side account/project remediation rather than another repo-local code wave.
...
- Done: `/status?format=json` now exports `provider_truth.rate_limit_reset_times` for live Gemini model cooldowns, so the seat-level status JSON exposes the same per-model reset truth that already drove OpenCode export and merged quota rows.
- Done: operator `seat-smoke` now makes requested-model cooldown explicit instead of forcing the operator to infer it from seat-level `degraded_enabled`: the response exposes `requested_model_key`, `requested_model_limited`, `requested_model_recovery_at`, and the live `rate_limit_reset_times` map.
- Done: release metadata is now aligned for patch publish as `0.8.4`; `VERSION`, `CHANGELOG.md`, and `docs/CHANGELOG.ru.md` all describe the model-specific Gemini cooldown/export/operator diagnostics that landed after `0.8.3`.
- Done: warmed browser-auth Gemini seats that still converge to `provider_truth.state=missing_project_id` now stay export-visible as `routing.state=degraded_enabled` with `fallback project in use; provider truth missing project_id`, and their `provider_truth.quota.models[]` rows stay `routable=true` in both `/status` and OpenCode cached quota instead of drifting into a false seat-blocked picture.
- Done: stale browser-auth Gemini truth refresh is now proactive rather than late-by-one-poll. The poller refreshes seats inside the next `10m` lead window before `fresh_until`, which prevents ready seats from falling out of routing as `stale_provider_truth` between scheduled refresh ticks.
...
- Note: a shared local OpenCode smoke can currently trip `SQLiteError: database is locked` when another long-lived `opencode` process owns the default local state DB, but the same smoke passes through the pool on an isolated `XDG_CONFIG_HOME`/`XDG_DATA_HOME`, so this is classified as local OpenCode state contention rather than a Gemini pool runtime regression.
...
**Verify hook:** `cd /home/lap/projects/codex-pool-orchestrator && go test -count=1 -run 'TestCandidate.*Gemini|TestRoutingState.*Gemini|TestBuildPoolDashboardData.*Gemini|TestBuildOpenCodeConfigBundle.*|TestOperatorGeminiSeatSmoke.*|TestLoadAccountsQuarantinesLongDeadAccount|TestServeStatusPageReturnsJSONForFormatQuery|TestServeFriendLanding_LocalTemplateIncludesCodexOAuthAction' ./... && curl -fsS http://127.0.0.1:8989/status?format=json | jq '{gemini_pool:.gemini_pool,gitlab_claude_pool:.gitlab_claude_pool,accounts:[.accounts[]|select((.type=="gemini") or (.type=="claude" and .plan_type=="gitlab_duo"))|{id,type,health_status,dead,routing:.routing,provider:.provider_truth,operational:.operational_truth}]}' && jq '{activeIndex,accounts:[.accounts[]|{email,enabled,lastSwitchReason,cachedQuota}]}' /home/lap/.config/opencode/pool-gemini-accounts.json && timeout 180s opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T57_LIVE_OK.'`

#### REPO-CPO-ALIGN-P2-T56: Make OpenCode via `codex-pool/gemini-3.1-pro-high` the canonical no-prompt local entrypoint
1. OpenCode is now the documented first-class local client path for this pool rather than one of several competing setup paths. The landing page, README, exported config bundle, and local wrapper all align on `codex-pool/gemini-3.1-pro-high` as the canonical operator-facing model name.
2. The generated OpenCode state is now explicit instead of implicit: the operator-facing invocation is `opencode run -m codex-pool/gemini-3.1-pro-high`, and the verified live pool snapshot kept `activeIndex=0` on a ready Gemini seat behind the exported `http://127.0.0.1:8989/v1` bundle.
3. The no-prompt local operator contract is live in this environment, not just documented. `opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T56_DEFAULT_OK.'` now routes through the pool without manual API-key repair.
...
- Done: in this environment, the canonical operator-facing invocation is `opencode run -m codex-pool/gemini-3.1-pro-high ...`; the legacy `agcode` wrapper is treated only as a local convenience shim.
- Done: the exported OpenCode bundle still points at `http://127.0.0.1:8989/v1` and keeps `activeIndex=0` on a ready Gemini seat.
- Done: the live local UX proof is complete. The landing now advertises OpenCode against `codex-pool/gemini-3.1-pro-high`, the current pool snapshot kept `activeIndex=0` on a ready seat, and `opencode run -m codex-pool/gemini-3.1-pro-high` returned `T56_DEFAULT_OK`.

**Verify hook:** `cd /home/lap/projects/codex-pool-orchestrator && go test -count=1 -run 'TestBuildOpenCodeConfigBundle|TestServeFriendLanding_LocalTemplateIncludesCodexOAuthAction|TestServeOpenCodeSetupScript_Bash|TestServeOpenCodeSetupScript_PowerShell' ./... && timeout 120s opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T56_DEFAULT_OK.' && jq -e '.accounts[.activeIndex].enabled == true' /home/lap/.config/opencode/pool-gemini-accounts.json >/dev/null && rg -n 'OpenCode|codex-pool/gemini-3.1-pro-high' README.md templates/local_landing.html ACTION_PLAN.md PROJECT_MANIFEST.md`
...
3. The pooled contract was re-proved after the operator fix instead of being assumed: `opencode run -m codex-pool/gemini-3.1-pro-high ...` still succeeds, so operator smoke and the real pooled `3.1 Pro High` path now agree on the same healthy runtime truth.
...
- Done: pooled `opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T58_OPENCODE_OK.'` passed after the same restart, proving the operator path no longer diverges from the live pooled contract.

**Verify hook:** `cd /home/lap/projects/codex-pool-orchestrator && go test -count=1 -run 'TestOperatorGeminiSeatSmoke.*|TestMaybeBuildGeminiCodeAssistFacadeRequest|TestTransformGeminiCodeAssistSSE|TestMaybeTransformGeminiCodeAssistFacadeResponseBuffered' ./... && STATUS_JSON=$(mktemp) && curl -fsS http://127.0.0.1:8989/status?format=json -o "$STATUS_JSON" && mapfile -t READY_IDS < <(jq -r '[.accounts[]|select(.type=="gemini" and (.provider_truth.state // "")=="ready")][0:2]|.[].id' "$STATUS_JSON") && test "${#READY_IDS[@]}" -ge 2 && for ACCOUNT_ID in "${READY_IDS[@]}"; do curl -fsS -X POST http://127.0.0.1:8989/operator/gemini/seat-smoke -H 'Content-Type: application/json' --data "{\"account_id\":\"${ACCOUNT_ID}\",\"model\":\"gemini-3.1-pro\",\"force_refresh\":false}" | jq -e '.generate.ok == true and ((.operational_truth.state // "") == "clean_ok")' >/dev/null; done && timeout 120s opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T58_OPENCODE_OK.'`
...
2. The export/client half of the proof also held on the fresh state: the OpenCode bundle stayed pointed at `http://127.0.0.1:8989/v1` without manual key repair, and `activeIndex=0` now points at the newly added ready seat.
3. The proof surfaced a real runtime contradiction without silently flattening it away: operator `seat-smoke` on `gemini-3.1-pro` returns typed `404 Not Found` for both the new ready seat and an older ready seat, while the new seat answers cleanly on `gemini-2.5-flash` and pooled `opencode run -m codex-pool/gemini-3.1-pro-high` still succeeds. That contradiction is split into successor bug card `T58` instead of reopening the release card.
...
- Done: the same state kept the OpenCode export clean; `pool-gemini-accounts.json` now places the fresh ready seat at `activeIndex=0`, and pooled `opencode run -m codex-pool/gemini-3.1-pro-high` returned `T55_OPENCODE_OK`.
...
**Verify hook:** `STATUS_JSON=$(mktemp) && curl -fsS http://127.0.0.1:8989/status?format=json -o "$STATUS_JSON" && ACCOUNT_ID="$(jq -r '[.accounts[]|select(.type=="gemini" and ((.operator_source // "") | ascii_downcase | test("browser")))]|max_by((.provider_truth.checked_at // .last_refresh_at // ""))|.id // empty' "$STATUS_JSON")" && test -n "$ACCOUNT_ID" && jq '{gemini_pool:.gemini_pool,accounts:[.accounts[]|select(.type=="gemini")|{id,operator_source,health_status,routing:.routing,provider:.provider_truth,operational:.operational_truth}]}' "$STATUS_JSON" && curl -fsS -X POST http://127.0.0.1:8989/operator/gemini/seat-smoke -H 'Content-Type: application/json' --data "{\"account_id\":\"$ACCOUNT_ID\",\"model\":\"gemini-2.5-flash\",\"force_refresh\":false}" | jq '{account_id,provider_truth_state,operational_truth,routing_state,load_code_assist,generate}' && timeout 120s opencode run -m codex-pool/gemini-3.1-pro-high 'Reply with exactly T55_OPENCODE_OK.'`

#### REPO-CPO-REL-P1-T54: Cut one combined Gemini/browser-auth/OpenCode + observability release
...
2. Final verification is complete on the release candidate binary: full `go build` / `go test`, strict manager status, live `/healthz` / `/status`, live OpenCode and direct Gemini API-key probes, repeated Gemini reset/rollback proof, and live GitLab Claude recovery smoke all passed on `2026-03-27`.
...
- Done: the same post-restart proof bundle re-confirmed the client contract against the restored seats: direct Gemini API-key setup stayed in external API-key mode on the pool root URL, OpenCode export stayed on `/v1` with `activeIndex=0` on an enabled seat, and both live probes returned `GEMINI_LIVE_OK` / `OPENCODE_LIVE_OK`.
...
2. Account rows and summary cards are now browser-auth-aware instead of managed-OAuth-biased: imported/browser-auth lanes show the right labels, `provider_quota_summary`, `compatibility_lane`, and `provider_truth.project_id`, and the landing consumes `gemini_pool` directly.
...
#### REPO-CPO-ALIGN-P1-T50: Align direct Gemini API-key access and OpenCode connectivity with the pool contract
1. Direct Gemini API-key access and OpenCode now have explicit, different sync/export semantics on the same running pool: the Gemini API-key lane uses the pool root URL in external API-key mode, while OpenCode uses the Anthropic-style `/v1` export plus `pool-gemini-accounts.json`.
...
Completion note (2026-03-27):
- Done: the post-restart probe wrote `.gemini/settings.json` with `selectedType=gemini-api-key`, `useExternal=true`, and `codeAssistEndpoint=http://127.0.0.1:8989`, proving the direct Gemini API-key client contract stays on the pool root URL rather than `/v1`.
- Done: the same probe wrote `~/.config/opencode/opencode.json` and `pool-gemini-accounts.json` with `provider_id=codex-pool`, `base_url=http://127.0.0.1:8989/v1`, `activeIndex=0`, and a disabled `missing_project_id` seat that no longer steals the active slot.
...
**Verify hook:** `cd /home/lap/projects/codex-pool-orchestrator && go test -count=1 -run 'Test.*Gemini.*Quota.*|TestCandidate.*Gemini|TestRoutingState.*Gemini|TestBuildPoolDashboardData.*|TestServeStatusPageReturnsJSONForFormatQuery|Test.*Gemini.*TruthRefresh.*|TestOperatorGeminiSeatSmoke.*' ./... && curl -fsS http://127.0.0.1:8989/status?format=json | jq '{gemini_pool:.gemini_pool,accounts:[.accounts[]|select(.type=="gemini")|{id,eligible:(.routing.eligible//false),block_reason:(.routing.block_reason//null),health_status:(.health_status//null),provider:(.provider_truth.state//null),freshness:(.provider_truth.freshness_state//null),routing:(.routing.state//null)}]}'`

#### REPO-CPO-ARCH-P1-T49: Freeze the Gemini quota-first contract after the browser-auth audit
...
2. Allowlisted browser-auth Gemini restrictions no longer masquerade as hard operational blocks on reload: live `/status?format=json` now publishes `health_status=restricted` with `routing.state=degraded_enabled`, while operator smoke separately records `operational_truth.state=degraded_ok` once a seat actually answers.
...
1. Extend the managed Gemini seat schema/load-save path so one seat can persist the provider-truth fields needed for later routing and facade admission: provider project id, `subscription_tier`, `protected_models`, `validation_blocked*`, per-model quota/reset snapshots, refresh timestamps, protocol caps (`max_output_tokens`, `thinking_budget`), and normalized provider block-state fields instead of external sidecars.
2. Add a Go-native Gemini warmup/probe contract on add and periodic refresh so a seat is only fully eligible after auth recovery plus the first provider-truth snapshot, and make that eligibility explicit enough for both `/status?format=json` and the local OpenCode/browser-auth client lane instead of relying on `index + sqlite + log-scrape` helpers.
...
#### REPO-CPO-ALIGN-P1-T48: Route pooled direct Gemini API-key traffic through our Code Assist facade
1. Copy the browser-auth-compatible Gemini content lane into the pool orchestrator itself by translating pooled Gemini `/v1beta/models/*:generateContent|streamGenerateContent` requests into Google Code Assist `v1internal` calls whenever the selected imported seat carries a provider project id.
2. Preserve the orchestrator's own pool-user admission and auth shape: Gemini clients keep using the pool through the synthetic `x-goog-api-key` lane, while upstream auth stays on imported browser-auth Gemini OAuth seats and responses are unwrapped back into Gemini API shape.
...
3. Keep it bounded to operator/runtime truth only: env-backed client discovery, seat-source labeling, and migration or sanity for older managed seats that depended on repo-bundled defaults; browser-auth quota or rotation semantics stay behind `T46` and `T47`.
