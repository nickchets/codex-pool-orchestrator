...
Закрыть текущую Gemini/Antigravity/OpenCode волну `T57` полностью на уровне репозитория, без ложных открытых хвостов в board/docs/git.

## In Scope

- подтвердить, что все repo-local изменения `T57` уже реализованы, проверены и опубликованы в `0.8.5`;
...
3. `agcode` smoke через пул проходит на свежем бинаре.
...
env XDG_CONFIG_HOME="$(mktemp -d)" XDG_DATA_HOME="$(mktemp -d)" AGCODE_RECREATE_USER=1 timeout 180s agcode run 'Reply with exactly T57_CLOSE_OK.'
